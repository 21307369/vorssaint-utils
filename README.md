...
- **Command Bar.** One shortcut opens a field over whatever you are doing. Type a few letters to run any Vorssaint action, open an app, switch to an open window, jump to a Settings page, insert a snippet or paste from the clipboard history at the cursor. It also reaches into the app you are using and runs any command from its menus, showing that command's own shortcut, so things buried three menus deep are one search away. Sums and unit conversions are answered as you type, numbers work as arguments (brightness 40), questions like battery or storage answer themselves, and an emoji goes straight into what you are writing. And it bends to you: press ⌘K on any result to give it your own name, pin it to the top, hide it for good or make the bar forget it, and turn off whole kinds of result you never want to see. It acts on what you already have selected too: change its case, clean a link, keep it or count it without copying anything first. You can save your own shortcuts (a site, a folder, or a search that takes whatever you type after its name), give any row a global shortcut of its own, and ask it for dates and clocks, like three weeks from now or the time in another city. It opens showing everything it can do, grouped by area, so the whole app is one scroll away, and every switch in Vorssaint can be flipped from it without opening Settings. It learns what you use most and forgets everything you type. Everything happens on your Mac: no account, no cloud, nothing sent anywhere.
